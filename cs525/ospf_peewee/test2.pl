...
# usage: test2.pl topFile command inf

# supported commands: ping, tr, web, unreach, log

@allinf = (
'v1-eth0', 'v1-eth1', 'v1-eth2', 
'v2-eth0', 'v2-eth1', 'v2-eth2',
'v3-eth0', 'v3-eth1', 'v3-eth2', 
'app1', 'app2');
...
foreach $f(@ifs) {
    ...
    if($command eq 'web') {
        ...
        `curl "http://$ip/index.html" -o /dev/null`;
        `curl "http://$ip:8888/Earl_Johnson--Aint_Nobodys_Business.mp3 -o /dev/null" `;
        `/bin/rm -f index.html Earl_Johnson--Aint_Nobodys_Business.mp3`;
